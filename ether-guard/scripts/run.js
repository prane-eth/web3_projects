const main = async () => {
	const contractFactory = await hre.ethers.getContractFactory("EtherGuard");
	...
	console.log(
		"Account Balance: ",
		hre.ethers.utils.formatEther(await contract.getBalance())
	);
	...
	tx = await contract.transferToAccount(
		addr1.address,
	...
	tx = await contract.transferToWallet(
		addr1.address,
	...
	// payToAccount
	await accountBalance(addr1);
	tx = await contract.payToAccount(addr1.address, {
	...
	// payToWallet
	await walletBalance(addr1);
	tx = await contract.payToWallet(addr1.address, {
		value: hre.ethers.utils.parseEther("0.5"),
	});
	await tx.wait();
	await walletBalance(addr1);
